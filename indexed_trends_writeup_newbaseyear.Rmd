...
This document is the same as the indexed_trends_writeup document, except now the base year has been set to 2012. 

**Because of this, much of the text below now needs to be updated.**
...
base_year <- 2012 # Change this as needed (Note this has to be an integer as the data are from HMD for international comparability)
...
dta %>% 
  ...
  mutate(index = 100 * mr / mr[year == base_year]) %>% # this produced the indices for ages in single years
  ungroup() %>% 
...
Figures showing how the weighted indices compared with 2012 values are shown separately for females and males below. 
...
indices_weighted %>% 
  ...
  labs(
    ...
    subtitle = "Base year: 2012. Weighted by population size within each age strata", 
    caption = "Source: Human Mortality Database",
    x = "Year", 
    y = "Index value (100 = 2012 rates)",
    colour = "Age group", 
    linetype = "Age group",
    size = "Age group"
  ) + 
  scale_y_continuous(limits = c(80, 320), breaks = seq(80, 320, by = 20))

ggsave("figures/age_trends_females_weighted_2012baseyear.png", height = 25, width = 25, units = 'cm', dpi = 300)
```
...
indices_weighted %>% 
  ...
  scale_y_continuous(limits = c(80, 200), breaks = seq(80, 200, by = 10))

ggsave("figures/age_trends_males_weighted_2012baseyear.png", height = 25, width = 25, units = 'cm', dpi = 300)
...
indices_weighted %>% 
  ...
  knitr::kable(
    digits = 0, 
    caption = "Weighted index of mortality rates in 0-14 year olds (2012 = 100), selected countries."
  ) %>% 
...
indices_weighted %>% 
  ...
  knitr::kable(
    digits = 0, 
    caption = "Weighted index of mortality rates in 15-34 year olds (2012 = 100), selected countries."
  ) %>% 
  kableExtra::kable_styling() 
...
indices_weighted %>% 
  ...
  knitr::kable(
    digits = 0, 
    caption = "Weighted index of mortality rates in 35-54 year olds (2012 = 100), selected countries."
  ) %>% 
  kableExtra::kable_styling() 
...
indices_weighted %>% 
  ...
  knitr::kable(
    digits = 0, 
    caption = "Weighted index of mortality rates in 55-74 year olds (2012 = 100), selected countries."
  ) %>% 
  kableExtra::kable_styling() 
...
indices_weighted %>% 
  ...
  knitr::kable(
    digits = 0, 
    caption = "Weighted index of mortality rates in 75-89 year olds (2012 = 100), selected countries."
  ) %>% 
  kableExtra::kable_styling() 
